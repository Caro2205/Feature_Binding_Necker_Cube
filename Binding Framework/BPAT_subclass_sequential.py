...
class BPAT_SEQUENTIAL(BPAT_Inference):

    """
    	Subclass of BPAT inference. 

        Performs BPAT inference in sequential manner,
        i.e. parameters are infered one after another, one every tuning cycle. 
    ...
    def __init__(self):
        ...
        self.param_infer_order = []
        self.num_infer_params = 0
       
        

    def iter_infer_param(self, idx_infer_param):
        infer_param = self.param_infer_order[idx_infer_param]
        print(f'---- infer {infer_param}')
        
        return (idx_infer_param + 1) % self.num_infer_params, infer_param


    def set_bools_to_infer_param(self, infer_param):     
        # TODO enum?? 
        do_binding = True if infer_param == 'bind' else False
        do_rotation = True if infer_param == 'rot' else False
        do_translation = True if infer_param == 'trans' else False

        return do_binding, do_rotation, do_translation
    ...
    def run_inference(self, 
        ...
        """
            ...
            Parameters: 
                ...
                - grad_calculations: list of size 3
                    indicators for the computation of the gradients 
                    out of: 
                    'lastOfTunHor', 'meanOfTunHor', 'weightedInTunHor'
        ...
        if do_binding:
            self.init_binding()
            self.param_infer_order.append('bind')
            
        ###########################  ROTATION  ################################
        if do_rotation:
            self.init_rotation()
            self.param_infer_order.append('rot')

        ###########################  TRANSLATION  #############################
        if do_translation:
            self.init_translation()
            self.param_infer_order.append('trans')

        #######################################################################
        self.num_infer_params = len(self.param_infer_order)
        next_idx_infer_param, infer_param = self.iter_infer_param(next_idx_infer_param)
        infer_binding, infer_rotation, infer_translation = self.set_bools_to_infer_param(infer_param)
        # do_binding, do_rotation, do_translation = self.set_bools_to_infer_param(infer_param)
        #######################################################################
